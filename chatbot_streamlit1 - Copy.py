...
st.title('PaniGenie')
st.write("A chatbot for the websit inidawaterportal.org. Ask any questions regarding water, water resources, water management in India.")

# Function to handle sending the message
def send_message():
    if 'user_input' in st.session_state and st.session_state.user_input:
        # Get chatbot response (replace 'qa.run' with your actual function)
        response = qa.run(st.session_state.user_input)
...
# Always display the text area for the response, but it will be empty if there is no response
response_placeholder.text_area("Chatbot says:", value=st.session_state.response, height=200, disabled=True)

# User input text box. The key parameter creates a unique identifier for the widget to track its state
user_input = st.text_input("Ask your question:", key="user_input", on_change=send_message, value=st.session_state.user_input)

# Button to clear the response area
if st.button("Clear Response", on_click=clear_response):
    # This will clear the text inside the response area but keep the box visible
    response_placeholder.text_area("Chatbot says:", value=' ', height=200, disabled=True)